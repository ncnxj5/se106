...
void Building(citymap &map,citymap & cur_map,int x,int y,int pre_pop,citymap & best)
{
	//char temp;
	//cur_map.print_map();
	...
	{
		{
			cur_map.put(best.board[y][x],x,y);
			if(if_next)
			{
			Building(map,cur_map,next_x,next_y,best.board[y][x],best);
			cur_map.put(EMPTY,x,y);
			if(JUMP)
			{
				if((x!=dest_x||y!=dest_y))
...
int Build_city(citymap &map,citymap &best)
{
	JUMP=false;
	IF_RED=false;
	SETAVE(map);
	citymap cur_map(map);//copy the current map as the temp
	Building(map,cur_map,0,0,0,best);
